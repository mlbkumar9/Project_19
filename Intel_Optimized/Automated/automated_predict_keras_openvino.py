...
# --------------------------------------------------------------------------
# This script expects the backbone as a command-line argument
# --------------------------------------------------------------------------
if len(sys.argv) > 1:
    BACKBONE = sys.argv[1]
else:
    print("Error: Keras backbone not provided as a command-line argument.")
    sys.exit(1)
...
OUTPUT_DIR = os.path.join(BASE_DIR, 'Predictions', 'Keras', f'unet-plus_{BACKBONE}_onnx')
ONNX_MODEL_PATH = os.path.join(BASE_DIR, 'Trained_Models', 'Keras', f'kuc_unet-plus_{BACKBONE}.onnx')
...
def main():
    ...
    for filename in image_files:
        try:
            ...
            predicted_mask = predicted_mask_onnx[0, :, :, 0] if predicted_mask_onnx.ndim == 4 else predicted_mask_onnx[0, 0, :, :]
